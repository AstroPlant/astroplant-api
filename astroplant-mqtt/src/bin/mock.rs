use astroplant_mqtt::{ServerRpcRequest, KitRpc};
...
fn main() {
    ...
    std::thread::spawn(move || {
        ...
        println!("Version response: {:?}", futures::executor::block_on(kit_rpc.version()));
        println!("Uptime response: {:?}", futures::executor::block_on(kit_rpc.uptime()));
    });
        ...
        if let astroplant_mqtt::MqttApiMessage::ServerRpcRequest(rpc_request) = res {
            match rpc_request {
                ServerRpcRequest::Version { response } => {
                    response.send("astroplant-mqtt-bin-tester".to_owned()).unwrap();
                }
                _ => {}
            }
        }
